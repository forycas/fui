<template>
  <div class="mt-1 relative" v-click-outside="closeSelect">
    <div ref="trigger"
         ...
         aria-expanded="true"
         @click="openSelect"
         class="w-full border rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default sm:text-sm"
         :class="{
              'bg-white border-gray-300': styling === STYLING_CHOICES.DEFAULT,
              'bg-primary-700 text-white border-primary-900': styling === STYLING_CHOICES.DARK
            }">
      <input ref="searchInput"
             ...
             :class="{
              'bg-white': styling === STYLING_CHOICES.DEFAULT,
              'bg-primary-700 text-white': styling === STYLING_CHOICES.DARK
            }"
             v-if="searchable" />
      ...
      <span class="ml-3 absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
        <svg class="h-5 w-5"
             :class="{
               'text-gray-400': styling === STYLING_CHOICES.DEFAULT,
               'text-white': styling === STYLING_CHOICES.DARK
             }"
...
export default defineComponent({
  ...
  props: {
    ...
    styling: {
      ...
      default: 'default',
      validator: (value: string): boolean => Object.values(STYLING_CHOICES).includes(value)
    }
  },
  setup (props, { emit }) {
    // Refs
    ...
    function getOptionItemClass (option: SelectOption): string {
      ...
      if (props.styling === STYLING_CHOICES.DARK) {
      ...
      return isActiveOption
        ? 'bg-primary-600 hover:bg-primary-800 text-white font-semibold'
        : 'bg-white hover:bg-primary-700 hover:text-white text-gray-900'
    }
